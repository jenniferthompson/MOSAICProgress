...
```{r screening}
# ## With plotly v 4.7, sizing got weird; switch to native plotly
# screen_plot <- ggplot(data = screening_summary, aes(x = myear)) +
#   geom_bar(aes(y = Screened),
#            fill = mosaic_col("red5"), stat = "identity", alpha = 0.75) +
#   geom_bar(aes(y = Approached),
#            fill = mosaic_col("orange3"), stat = "identity", alpha = 0.75) +
#   geom_bar(aes(y = Enrolled),
#            fill = mosaic_col("green4"), stat = "identity", alpha = 0.75) +
...
x_screen <- list(tickvals = screening_summary$myear,
                 ticktext = screening_summary$myear_char,
                 title = "")
y <- list(title = "")
...
#            # width = screenplot_wd, height = screenplot_ht) %>%
#   layout(xaxis = x, yaxis = y)
...
### Zarit Caregiver Completion Rate

```{r zarit_full}
pct_zarit_full <-
  round(mean(all_enrolled$zarit_comp_raw == "Fully completed", na.rm = TRUE) * 100)
pct_zarit_fullpartial <-
  round(mean(all_enrolled$zarit_comp_raw %in%
               paste(c("Fully", "Partially"), "completed"),
             na.rm = TRUE) * 100)

valueBox(
  value = paste0(pct_zarit_full, "%"),
  caption = "patients with fully completed Zarit",
  color = ifelse(pct_zarit_full < 80, mosaic_col("orange3"), mosaic_col("green3")),
...
Column {data-width=500}
-----------------------------------------------------------------------
### **Accelerometer Central** {data-height=30}
...
accel_text <- ifelse(
  days_no_accel == 0,
  "days accelerometer removed >=1 time",
  paste0(
...
valueBox(
  ...
  caption = "patients with accelerometer permanently removed",
...
Technical Info
=====================================
R version `r devtools::session_info()$platform$version`. Packages:
